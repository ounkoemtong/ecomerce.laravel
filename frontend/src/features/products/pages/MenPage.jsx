...
export default function MenPage() {
  ...
  // Filter products by 'Men' first, then apply user filters
  const menProducts = useMemo(() => {
    return products.filter(product => {
      const cats = getProductCategories(product)
      return cats.includes('Men')
    })
  }, [products])

  // Dynamic list of product types (categories) available in Men's collection
  ...
  const filteredProducts = useMemo(() => {
    ...
    return result
  }, [menProducts, selectedType, priceRange, selectedSize, sortBy])
      ...
      <main className="mx-auto max-w-7xl px-4 pb-24 pt-20 sm:px-6 lg:px-8">
        ...
        <section className="relative h-[60vh] min-h-[400px] overflow-hidden border border-amber-500/20 mb-12 group">
          ...
          <div className="absolute bottom-10 left-8 md:left-12 text-white max-w-xl">
            <p className="text-[0.7rem] uppercase tracking-[0.4em] text-amber-200/80 mb-2 font-semibold">Atelier Men</p>
            <h1 className="font-serif text-4xl sm:text-5xl tracking-wide leading-tight mb-4">Sartorial Precision</h1>
            <p className="text-sm leading-relaxed text-stone-300">
              Sculptured heavy virgin wool overcoats, refined tailored trousers, pure cashmere layers, and hand-finished boots for a sharper daily silhouette.
            </p>
          </div>
        </section>
            ...
            <>
              <div className="grid gap-6 grid-cols-2 md:grid-cols-4">
                {paginatedProducts.map((product) => {
                  ...
                  if (product.id === 'polished-leather-boots') badge = 'Limited Edition'
