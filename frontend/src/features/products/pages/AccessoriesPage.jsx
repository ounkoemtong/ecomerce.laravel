...
export default function AccessoriesPage() {
  ...
  // Filter products by 'Accessories' first, then apply user filters
  const accessoriesProducts = useMemo(() => {
    return products.filter(product => {
      const cats = getProductCategories(product)
      return cats.includes('Accessories')
    })
  }, [products])

  // Dynamic list of product types (categories) available in Accessories collection
  ...
  const filteredProducts = useMemo(() => {
    let result = [...accessoriesProducts]
    ...
    return result
  }, [accessoriesProducts, selectedType, priceRange, selectedSize, sortBy])
      ...
      <main className="mx-auto max-w-7xl px-4 pb-24 pt-20 sm:px-6 lg:px-8">
        ...
        <section className="relative h-[60vh] min-h-[400px] overflow-hidden border border-amber-500/20 mb-12 group">
          ...
          <div className="absolute bottom-10 left-8 md:left-12 text-white max-w-xl">
            <p className="text-[0.7rem] uppercase tracking-[0.4em] text-amber-200/80 mb-2 font-semibold">Atelier Accessories</p>
            <h1 className="font-serif text-4xl sm:text-5xl tracking-wide leading-tight mb-4">Purposeful Texture</h1>
            <p className="text-sm leading-relaxed text-stone-300">
              Clean geometric leather bags, asymmetric strap heels, and handcrafted objects to define personal uniform rituals.
            </p>
          </div>
        </section>
          ...
          <section className={`p-6 border-b grid gap-6 grid-cols-1 sm:grid-cols-3 mb-8 transition-all duration-300 ${
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {['All', 'One Size', '36', '37', '38', '39'].map(sz => (
                  <button
                    key={sz}
            ...
            <>
              <div className="grid gap-6 grid-cols-2 md:grid-cols-4">
                {paginatedProducts.map((product) => {
                  ...
                  if (product.id === 'structured-leather-bag') badge = 'Best Seller'
                  if (product.id === 'gold-hoop-earrings') badge = 'Limited Edition'
